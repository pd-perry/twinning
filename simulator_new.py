...
class RetailerEnv(gym.Env):
    ...
    def demand(self, s_t, action, p_t):
        #TODO: review
        μ = self.beta[s_t, 0] + self.beta[s_t, 1] * (1 - action[0]) * p_t
        minimum = 1 
        maximum = 1000
        return self.truncated_poisson(μ, minimum, maximum)


    def truncated_poisson(self, μ, minimum, maximum):
        temp = 0 
        while temp < minimum or temp > maximum: 
            temp = sct.poisson.rvs(μ)
        return temp 
